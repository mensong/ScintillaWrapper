...
CScintillaEditState::CScintillaEditState() noexcept : bRegularExpression(FALSE),
...
#pragma warning(suppress: 26409 26440 26433)
...
void CScintillaView::OnUpdateNeedTextAndFollowingText(CCmdUI* pCmdUI)
{
  ...
  CScintillaCtrl& rCtrl = GetCtrl();
  const int nLength = rCtrl.GetTextLength();
  const Sci_Position nStartChar = rCtrl.GetSelectionStart();
...
#pragma warning(suppress: 26490)
  ...
  const int nLength = rCtrl.GetLength();
  if (bNext)
...
{
  CScintillaCtrl& rCtrl = GetCtrl();
  const Sci_Position index = rCtrl.FindText(dwFlags, &ft);
...
#pragma warning(suppress: 26434)
BOOL CScintillaView::OnEraseBkgnd(CDC* /*pDC*/) noexcept
{
...
#pragma warning(suppress: 26429)
  ...
  else
  {
    ...
    const int nDocLength = rCtrl.GetLength();
...
#pragma warning(suppress: 26429)
    ...
    for (int i=0; i<nDocLength; i+=4095) //4095 because data will be returned nullptr terminated
    {
      ...
      //Write it to disk
      pFile->Write(tr.lpstrText, nGrabSize);
...
void CScintillaView::OnMarginClick(_Inout_ SCNotification* pSCNotification)
{
  ...
  CScintillaCtrl& rCtrl = GetCtrl();
  const int nLine = rCtrl.LineFromPosition(pSCNotification->position);
